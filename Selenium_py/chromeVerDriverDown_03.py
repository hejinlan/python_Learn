...
class sd_chrome():
    def __init__(self,_folder=r'f:\05.Python\chromedriver_win32V88'):
        # 防止文件夹路径不存在
        # 指定下载路径文件夹
    ...
    def replaceDriverInPython(self):
        ...
        for path in sys.path:
            if path.split('\\')[-1] == 'Python38-32' and os.path.isdir(path):
                pypath = path + '\Lib\site-packages\selenium\webdriver\chrome'
            if pypath != '':
                break    
            ...
            if path.split('\\')[-1] == 'Python38-32' and os.path.isdir(path):
                for d in os.listdir(path):
                    if os.path.isdir(os.path.join(path,d)) and d == 'Lib':
                        ...
                        print(path) 
                        for d in os.listdir(path):
                            if os.path.isdir(os.path.join(path,d)) and d == 'site-packages':
                                pypath = '%s\\%s' % (path,d)
                                print(pypath) 
                                break
            if pypath != '':
...
if __name__ == '__main__':
	down = r'f:\05.Python\chromedriver_win32V88'
	chromeClass = sd_chrome(down)
	ChromeVersion = chromeClass.getChromeVersion()
	DriverVer = chromeClass.getChromeDriverVersionList()
	compare = chromeClass.compareVersion()
	download = chromeClass.downloadChromeDriver()
	unzip = chromeClass.unzipFile()
	replaceDriver = chromeClass.replaceDriverInPython()
	print(replaceDriver)
